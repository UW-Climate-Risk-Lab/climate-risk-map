"""
This module contains code from the climate-indices package. This was copied here
due to out of date numba versioning on pacakage causing unusable SPEI functions.
At the time of writing, updates to the climate-indices packages seem to be delayed.
Slight changes were made here to remove numba.jit decorators

*** ALL CREDIT AND ATTRIBUTION GOES TO https://climate-indices.readthedocs.io/en/latest/ ***
...
def reshape_to_2d(
    ...
    """
    **ADAPTED FROM climate-indices utils**
    ...
    if len(shape) == 2:
        if shape[1] == second_axis_length:
            ...
            message = (
                "Values array has an invalid shape (2-D but second "
...
def _validate_array(
    ...
    if len(values.shape) == 1:
        if periodicity is None:
            message = (
                "1-D input array requires a corresponding periodicity "
...
def pearson_parameters(
    ...
    for time_step_index in range(time_steps_per_year):
        ...
        number_of_zeros, number_of_non_missing = count_zeros_and_non_missings(
            time_step_values
        )
...
def gamma_parameters(
    ...
    if (np.ma.is_masked(values) and values.mask.all()) or np.all(np.isnan(values)):
        if periodicity is Periodicity.monthly:
            ...
            raise ValueError(
                "Unsupported periodicity: {periodicity}".format(periodicity=periodicity)
            )
...
def scale_values(
    ...
    # or to (years, 366) for daily
    if periodicity is Periodicity.monthly:
        scaled_values = reshape_to_2d(scaled_values, 12)

    elif periodicity is Periodicity.daily:
        scaled_values = reshape_to_2d(scaled_values, 366)
...
def sum_to_scale(
    ...
    """
    **ADAPTED FROM climate_indices package**
    https://github.com/monocongo/climate_indices
    ...
    # pad the first (n - 1) elements of the array with NaN val
    return np.hstack(([np.NaN] * (scale - 1), sliding_sums))
...
def spei(
    ...
    **ADAPTED FROM climate_indices package**
    ...
    if distribution is Distribution.pearson:
        ...
        transformed_fitted_values = transform_fitted_pearson(
            scaled_values,
            data_start_year,
...
def fit(timeseries: np.ndarray) -> dict:
    ...
    if (lmoments[1] <= 0.0) or (abs(lmoments[2]) >= 1.0):
        message = (
            "Unable to calculate Pearson Type III parameters "
...
def _estimate_pearson3_parameters(lmoments: np.ndarray) -> dict:
    ...
    if (lmoments[1] <= 0) or (t3 >= 1):
        message = (
            "Unable to calculate Pearson Type III parameters due to invalid L-moments"
        )
        raise ValueError(message)
