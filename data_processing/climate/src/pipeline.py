...
def load_historical_data(model: str, ensemble_member: str, required_vars: list[str], years: List[int]):
    ...
    for year in years:
        for var in required_vars:
            uri = file_utils.find_best_input_file(s3_client=s3_client, model=model, scenario='historical', ensemble_member=ensemble_member, year=year, variable=var)
            if uri:
    ...
    if missing_vars:
         ...
         raise FileNotFoundError(f"Missing required input URIs for variables: {missing_vars}")

    if not uris_to_load:
        raise ValueError("No input URIs provided to load_input_data.")

    print(f"Loading input data from: {uris_to_load}")
...
def load_input_data(config: PipelineConfig, required_vars: list[str]) -> xr.Dataset:
    ...
    if config.bbox:
        ...
        if ds_input.dims['lat'] == 0 or ds_input.dims['lon'] == 0:
             raise ValueError("Bounding box selection resulted in empty spatial dimensions.")

    # --- IMPORTANT: Rechunk to target chunks for calculations ---
    target_chunks = {
        "time": config.time_chunk if config.time_chunk > 0 else ds_input.dims["time"], # -1 means full dim
        "lat": config.lat_chunk,
        "lon": config.lon_chunk,
    }
    # Filter chunks for dimensions actually present
    target_chunks = {dim: size for dim, size in target_chunks.items() if dim in ds_input.dims}
...
def run_pipeline_for_year(config: PipelineConfig):
    ...
    all_output_vars_needed = [var for ind_cfg in constants.INDICATOR_REGISTRY.values() for var in ind_cfg['output_vars']]
    vars_exist_map = file_utils.check_vars_exist(config.zarr_output_uri, all_output_vars_needed)
    ...
    for name, ind_cfg in constants.INDICATOR_REGISTRY.items():
         ...
         if all(vars_exist_map.get(var, False) for var in ind_cfg['output_vars']):
              ...
              print(f"Indicator '{name}' needs calculation (missing: {[var for var in ind_cfg['output_vars'] if not vars_exist_map.get(var, False)]}).")
              ...
              all_req_vars.update(var for var in constants.VAR_LIST) # Simple update based on VAR_LIST
    ...
    if "fwi" in indicators_to_run and constants.INDICATOR_REGISTRY["fwi"]["requires_initial_conditions"]:
        prior_year = config.year - 1
        # Construct prior year's Zarr path (assuming same naming convention)
        prior_year_output_uri = config.zarr_output_uri.replace(f"_{config.year}.zarr", f"_{prior_year}.zarr")
        
        print(f"Checking for prior year FWI data for initial conditions: {prior_year_output_uri}")
        ...
        if all(prior_vars_exist.get(var, False) for var in ["ffmc", "dmc", "dc"]):
            try:
                ...
                ds_prior = xr.open_zarr(prior_year_output_uri, consolidated=True, chunks=None) 
                ...
                print(f"Warning: Failed to load initial conditions from {prior_year_output_uri}: {e}. Proceeding without them.")
                # FWI calculation function should handle None inputs
        else:
            print(f"Prior year Zarr {prior_year_output_uri} missing required FWI vars for initial conditions.")
    ...
    if "precip_percent_change" in indicators_to_run:
         ...
         try:
              ...
              ds_pr_historical = load_historical_data(model=args.model, ensemble_member=args.ensemble_member, required_vars=['pr'], years=constants.HISTORICAL_BASELINE_YEARS)
              ...
              print(f"ERROR: Failed to get precipitation baseline: {e}. Skipping precip_percent_change.")
    ...
    if not indicators_to_run:
        print(f"No indicators remaining to run for {config.year} after checking context requirements.")
        return
    ...
    for name, ind_cfg in indicators_to_run.items():
        ...
        try:
            ...
            if name == "fwi" and "fwi_initial_conditions" in config.indicator_context:
                 kwargs['initial_conditions'] = config.indicator_context['fwi_initial_conditions']
            if name == "precip_percent_change" and "precip_baseline_mean" in config.indicator_context:
                 kwargs['pr_baseline_mean'] = config.indicator_context['precip_baseline_mean']
    ...
    if results_to_merge:
        ...
        try:
             ...
             print(f"Warning: Error loading existing Zarr {config.zarr_output_uri} for merge: {e}. Starting fresh.")
        ...
        if 'ds_input' in locals(): del ds_input # Delete potentially large input ds
        gc.collect()
        ...
        try:
            ...
            raise # Fail the job if writing fails
             ...
             if 'ds_final' in locals(): del ds_final # Final cleanup
             gc.collect()
    ...
    print(f"--- Finished Processing for Year {config.year} in {year_elapsed_time:.2f} seconds ---")
...
def main(args):
    ...
    print(f"Setting up Dask LocalCluster: {n_workers} workers, {threads_per_worker} threads/worker, {memory_limit_gb}GB total memory limit")
    ...
    if all([args.x_min, args.y_min, args.x_max, args.y_max]):
        try:
             bbox = BoundingBox(x_min=float(args.x_min), y_min=float(args.y_min), 
                                x_max=float(args.x_max), y_max=float(args.y_max))
             print(f"Using bounding box: {bbox}")
        except ValueError:
             print("Warning: Invalid bounding box coordinates provided. Proceeding without bounding box.")
    else:
    ...
    for year in years:
        ...
        try:
            ...
            required_vars_all_indicators = set(var for var in constants.VAR_LIST) # Get unique var names
            for var_name in required_vars_all_indicators:
                 input_uri = file_utils.find_best_input_file(s3_client, args.model, args.scenario, args.ensemble_member, year, var_name)
                 if input_uri:
                      input_uris[var_name] = input_uri
                 else:
                      # Handle missing critical input files - maybe skip year or raise error?
                      print(f"CRITICAL WARNING: Could not find required input file for variable '{var_name}' for year {year}. Skipping year.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate Climate Indicators using Dask")
    ...
    parser.add_argument("--ensemble_member", type=str, required=True, help="Simulation Run e.g 'r1i1p1f1'")
    # Made chunk/resource args optional, using defaults from constants if not provided
    parser.add_argument("--lat_chunk", type=str, required=False, help=f"Size of latitude chunk (default: {constants.DEFAULT_LAT_CHUNK})")
    parser.add_argument("--lon_chunk", type=str, required=False, help=f"Size of longitude chunk (default: {constants.DEFAULT_LON_CHUNK})")
    parser.add_argument("--threads", type=str, required=False, help=f"Threads per Dask worker (default: {constants.DEFAULT_THREADS})")
    parser.add_argument("--memory_available", type=str, required=False, help=f"Total GB RAM available (default: {constants.DEFAULT_MEMORY}GB)")
    parser.add_argument("--x_min", type=str, required=False, help="Bounding box: minimum Longitude")
    parser.add_argument("--y_min", type=str, required=False, help="Bounding box: minimum Latitude")
    parser.add_argument("--x_max", type=str, required=False, help="Bounding box: maximum Longitude")
    parser.add_argument("--y_max", type=str, required=False, help="Bounding box: maximum Latitude")
