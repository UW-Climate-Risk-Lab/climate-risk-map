...
def main(
    ...
    num_rows = len(df)
    logger.info(f"Starting efficient bulk load for '{climate_variable}', ssp '{ssp_value}'. Rows: {num_rows}")
    ...
    idx_month_name = f"idx_{target_table_name}_on_month"
    idx_decade_name = f"idx_{target_table_name}_on_decade"
    idx_month_decade_name = f"idx_{target_table_name}_on_month_decade"
    idx_ssp_name = f"idx_{target_table_name}_on_ssp"
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(sql.SQL("""
                CREATE UNIQUE INDEX {index_name} ON {schema}.{target_table} (osm_id, month, decade, ssp);
            ...
            cur.execute(sql.SQL("CREATE INDEX {index_name} ON {schema}.{target_table} (month);")
                        .format(index_name=sql.Identifier(idx_month_name), schema=schema_ident, target_table=target_table_ident))
            logger.debug(f"Recreated {idx_month_name}")

            cur.execute(sql.SQL("CREATE INDEX {index_name} ON {schema}.{target_table} (decade);")
                        .format(index_name=sql.Identifier(idx_decade_name), schema=schema_ident, target_table=target_table_ident))
            logger.debug(f"Recreated {idx_decade_name}")

            cur.execute(sql.SQL("CREATE INDEX {index_name} ON {schema}.{target_table} (month, decade);")
                        .format(index_name=sql.Identifier(idx_month_decade_name), schema=schema_ident, target_table=target_table_ident))
            logger.debug(f"Recreated {idx_month_decade_name}")

            cur.execute(sql.SQL("CREATE INDEX {index_name} ON {schema}.{target_table} (ssp);")
                        .format(index_name=sql.Identifier(idx_ssp_name), schema=schema_ident, target_table=target_table_ident))
            logger.debug(f"Recreated {idx_ssp_name}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        main(
            ...
            climate_variable='fwi',     # Matches your table name part
            conn=connection
        )
