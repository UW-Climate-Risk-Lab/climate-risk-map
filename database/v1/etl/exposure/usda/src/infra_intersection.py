...
def zonal_aggregation_linestring_optimized(
    ...
    Returns:
        pd.DataFrame: A DataFrame with aggregated climate statistics (mean, min, max)
                      grouped by the original linestring ID (`id_column`). 
                      Returns an empty DataFrame if no valid data
                      can be processed.
    ...
    if not agg_dict_filtered:
        print(
            f"Error: None of the target columns found for aggregation in the DataFrame. "
    ...
    try:
        # Group by the original linestring ID
...
def create_pgosm_flex_query(
    ...
    table = f"unexposed_ids_usda_{usda_variable}"
...
def main(
    ...
    Args:
        ...
        usda_variable (str): Climate variable being queried
        crs (str): Coordinate Reference System
    ...
    """
    query, params = create_pgosm_flex_query(usda_variable=usda_variable, crs=crs)
    ...
    failed_aggregations = df.loc[df["burn_probability"].isna(), ID_COLUMN].nunique()
    logger.warning(
