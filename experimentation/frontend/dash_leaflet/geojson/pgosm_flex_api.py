...
class OpenStreetMapDataAPI:
    ...
    def _is_valid_geometry(self, geometry: Dict) -> bool:
        ...
        if geom_type == "Point":
            return isinstance(coordinates, list) and len(coordinates) == 2 and all(isinstance(coord, (int, float)) for coord in coordinates)
        elif geom_type == "MultiPoint":
            return isinstance(coordinates, list) and all(isinstance(coord, list) and len(coord) == 2 and all(isinstance(c, (int, float)) for c in coord) for coord in coordinates)
        elif geom_type == "LineString":
            return isinstance(coordinates, list) and all(isinstance(coord, list) and len(coord) == 2 and all(isinstance(c, (int, float)) for c in coord) for coord in coordinates)
        elif geom_type == "MultiLineString":
            return isinstance(coordinates, list) and all(isinstance(line, list) and all(isinstance(coord, list) and len(coord) == 2 and all(isinstance(c, (int, float)) for c in coord) for coord in line) for line in coordinates)
        elif geom_type == "Polygon":
            return isinstance(coordinates, list) and all(isinstance(ring, list) and all(isinstance(coord, list) and len(coord) == 2 and all(isinstance(c, (int, float)) for c in coord) for coord in ring) for ring in coordinates)
        elif geom_type == "MultiPolygon":
            return isinstance(coordinates, list) and all(isinstance(polygon, list) and all(isinstance(ring, list) and all(isinstance(coord, list) and len(coord) == 2 and all(isinstance(c, (int, float)) for c in coord) for coord in ring) for ring in polygon) for polygon in coordinates)
        elif geom_type == "GeometryCollection":
            return isinstance(coordinates, list) and all(self._is_valid_geometry(geom) for geom in coordinates)
        return False
    ...
    def _is_valid_feature(self, feature: Dict) -> bool:
        ...
        if "type" not in feature or feature["type"] != "Feature":
            return False
        if "geometry" not in feature or not self._is_valid_geometry(feature["geometry"]):
            return False
    ...
    def _is_valid_geojson(self, geojson: Dict) -> bool:
        ...
        if geojson_type == "FeatureCollection":
            if "features" not in geojson or not isinstance(geojson["features"], list):
                return False
            return all(self._is_valid_feature(feature) for feature in geojson["features"])
        elif geojson_type == "Feature":
            return self._is_valid_feature(geojson)
        elif geojson_type in ["Point", "MultiPoint", "LineString", "MultiLineString", "Polygon", "MultiPolygon", "GeometryCollection"]:
