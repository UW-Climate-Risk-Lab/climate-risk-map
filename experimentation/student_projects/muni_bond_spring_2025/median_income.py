...
CRS = "EPSG:4326"
def get_median_income_dataset() -> pd.DataFrame:
    """
    ...
    *Columns*
    ...
    s = df.groupby(["zipcode", "year"]).apply(lambda x: sum(x["A00100"]*1000) / sum(x["N1"]), include_groups=False)
    s.name = "weighted_average_adjusted_gross_income"
    df = s.reset_index()
    return df
...
def main():
    ...
    df_median_income = get_median_income_dataset()
    gdf_zip_code = gpd.read_file(ZIPCODE_SHAPE_FILE)
    gdf_zip_code = gdf_zip_code[["ZCTA5CE20", "geometry"]]
    gdf_zip_code["ZCTA5CE20"] = gdf_zip_code["ZCTA5CE20"].astype(int)
    gdf_zip_code = gdf_zip_code.to_crs(CRS)
    gdf_zip_code = gdf_zip_code.merge(df_median_income, how="left", left_on="ZCTA5CE20", right_on="zipcode")
    gdf_zip_code = gdf_zip_code.drop(columns=["ZCTA5CE20"])
    gdf_zip_code = gdf_zip_code.dropna()
    

    gdf_elec_service_areas = gpd.read_file(ELECTRIC_RETAIL_SERVICE_AREA_FILE)
    ...
    intersection_gdf = gpd.overlay(gdf_elec_service_areas, gdf_zip_code, how='intersection', keep_geom_type=True)
    ...
    intersection_gdf.loc[valid_areas, 'percent_of_zip_in_sa'] = \
        intersection_gdf.loc[valid_areas, 'intersection_area'] / intersection_gdf.loc[valid_areas, 'original_zip_area']

    # Ensure percentage is between 0 and 1 (due to potential floating point inaccuracies)
    intersection_gdf['percent_of_zip_in_sa'] = intersection_gdf['percent_of_zip_in_sa'].clip(0, 1)
    ...
    intersection_gdf['weighted_average_adjusted_gross_income_x_intersection_area'] = intersection_gdf['weighted_average_adjusted_gross_income'] * intersection_gdf['intersection_area']

    utility_area_weighted_income = intersection_gdf.groupby('NAME').agg(
        sum_income_x_intersection_area=('weighted_average_adjusted_gross_income_x_intersection_area', 'sum'),
        total_intersection_area_in_utility=('intersection_area', 'sum')
    ).reset_index()

    utility_area_weighted_income['area_weighted_adjuted_gross_income'] = \
        utility_area_weighted_income['sum_income_x_intersection_area'] / utility_area_weighted_income['total_intersection_area_in_utility']
    ...
    gdf_utilities_final = gdf_elec_service_areas.merge(
        utility_area_weighted_income[['NAME', 'area_weighted_adjuted_gross_income', 'total_intersection_area_in_utility']],
        on='NAME',
        how='left'
    )
