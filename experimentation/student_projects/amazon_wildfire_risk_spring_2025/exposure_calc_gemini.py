...
def calculate_distance_to_nearest_firestation(
    ...
    if infra_gdf.empty or firestations_gdf.empty:
        ...
        return pd.DataFrame({id_column: infra_gdf[id_column], 'Distance_to_FireStation_mi': np.nan}).set_index(id_column)
    ...
    try:
        ...
        joined_gdf['Distance_to_FireStation_mi'] = joined_gdf['distance_meters'] * 0.000621371
        ...
        joined_gdf['Distance_to_FireStation_mi'].replace([np.inf, -np.inf], np.nan, inplace=True)
        ...
        result_df = infra_gdf[[id_column]].merge(
             joined_gdf[['Distance_to_FireStation_mi']],
             left_index=True,
             right_index=True,
             how='left'
        )
...
def main():
    ...
    try:
        ...
        # Result has ID_COLUMN and Distance_to_FireStation_mi
    ...
    logger.info("Starting compute for relevant climate data subset for parallel processing.")
    climate_subset = climate_ds[RADIUS_DATA_VARS].compute()
    logger.info("Finished compute for relevant climate data subset for parallel processing.")
    radius_results_list = [] # Store results DataFrames for each radius
    for radius in RADIUS_MILES:
        ...
        columns_to_keep = [ID_COLUMN] + RADIUS_DATA_VARS
        ...
        radius_results_list.append(stats_df[existing_columns_to_keep])
    ...
    if not firestation_distance_df.empty:
        ...
        final_df = final_df.merge(firestation_distance_df[['Distance_to_FireStation_mi']],
        ...
        final_df['Distance_to_FireStation_mi'] = np.nan # Add column explicitly if empty
    ...
    if not point_df.empty:
        ...
        if 'month' in point_df.columns:
            merge_cols_point.append('month')
    ...
    for i, radius_df in enumerate(radius_results_list):
        radius = RADIUS_MILES[i] # Assuming order is maintained
