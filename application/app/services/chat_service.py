...
class ChatService:
    ...
    def _invoke_bedrock_agent(
        ...
        """
        logger.info(f"Invoking Bedrock Agent for session {session_id}. Input starts with: '{input_text[:10]}...'")
        text_response = ""

        try:
            # runtime_agent = ChatService._get_bedrock_runtime_client() # If using class method client
            runtime_agent = boto3.client(service_name="bedrock-agent-runtime") # Instantiate per call
            ...
            if not event_stream:
                logger.warning(f"No 'completion' event stream found in Bedrock response for session {session_id}.")
                return None, None
            ...
            for event in event_stream:
                if "chunk" in event:
                    ...
                    if "bytes" in chunk:
                     ...
                     logger.debug(f"Trace event received for session {session_id}") # Example
                elif "files" in event:
                    ...
                    logger.info(f"Received 'files' event for session {session_id}: {event['files']}")
            ...
            logger.info(f"Agent invocation successful for session {session_id}. Response length: {len(text_response)}")
            return text_response

        except runtime_agent.exceptions.ValidationException as e:
             logger.error(f"Bedrock Agent Validation Error for session {session_id}: {e}. Input: '{input_text[:100]}...'")
             return f"Agent Error: Input validation failed. {e}", None
        except runtime_agent.exceptions.ConflictException as e:
            logger.error(f"Bedrock Agent Conflict Error for session {session_id}: {e}. Maybe session issue?")
            # You might want to try creating a new session or inform the user
            return f"Agent Error: Session conflict. Please try closing and reopening the chat. {e}", None
        except Exception as e:
            logger.error(f"Error invoking Bedrock Agent for session {session_id}: {e}")
            return f"An unexpected error occurred while communicating with the AI agent: {e}", None
    ...
    def check_chat_criteria(
        ...
        hazard = HazardConfig.get_hazard(hazard_name=hazard_name) if hazard_name else None
        region = MapConfig.get_region(region_name=region_name) if region_name else None
        assets = [AssetConfig.get_asset(name=asset_label) for asset_label in asset_overlays if AssetConfig.get_asset(name=asset_label)]

        # --- Validation Checks ---
        missing_params = []
        if not region: missing_params.append("Region")
        if not hazard: missing_params.append("Hazard Indicator")
        ...
        if shapes is None or not shapes.get("features"): # Safer check for features
            chat_allowed_message = "Please select an area on the map (Hint: Use the drawing tools)."
            chat_allowed = False
            return chat_allowed, chat_allowed_message
        ...
        try:
            ...
            chat_allowed_message = "Could not calculate the area of the selected shapes."
            chat_allowed = False
            return chat_allowed, chat_allowed_message
    ...
    def invoke_ai_agent(
        ...
        if not user_input:
            ...
            return ChatMessage.create_message(role="ai", text="I didn't receive any input. Please try again.")
    ...
    def start_ai_session(
        ...
        try:
            ...
            logger.error(f"Error converting DataFrame to CSV for session {session_id}: {e}")
            return None, None
        ...
        if text is None: # Handle case where initial agent invocation failed
             logger.error(f"Initial agent invocation failed for new session {session_id}.")
             # Decide if we should still return the session ID or None
             return session_id, ChatMessage.create_message(role="ai", text="Sorry, I couldn't initialize the analysis session.", fig=None)
