...
INTERSECTION_DEBUG = False if os.environ["INTERSECTION_DEBUG"].lower() == "false" else True # TODO: REMOVE THIS
    ...
    with tempfile.TemporaryDirectory() as geotiff_tmpdir:
        if not INTERSECTION_DEBUG:
            ...
            utils.upload_files(
                s3_bucket=S3_BUCKET,
                s3_prefix=utils.create_s3_prefix(
                    S3_BASE_PREFIX, CLIMATE_VARIABLE, SSP, f"cogs/{TIME_AGGREGATION_METHOD}"
                ),
                dir=geotiff_tmpdir,
            )
